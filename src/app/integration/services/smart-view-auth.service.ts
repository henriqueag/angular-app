...
import { BehaviorSubject, catchError, first, interval, Subscription, switchMap, tap, throwError } from "rxjs";
import { environment } from "src/environments/environment";
import { SmartViewAuthToken } from './../models/smart-view-auth-token.model';

@Injectable()
export class SmartViewAuthService {
    private _token = new BehaviorSubject<SmartViewAuthToken | null>(null);
    ...
    private setupAutoRefreshToken() {
        ...
        const now = new Date();
        const expirationDate = new Date(this.token!.expires_at ?? 0);
